...
/** A GitHub Actions workflow, modeled as an algebraic data type.
  ...
  * (`runs-on`, `timeout-minutes`, `fail-fast`, ...). See [[Render]] for the one place derivation cannot reach: the `on:`
  * triggers block, whose keys (`pull_request`, `workflow_dispatch`, ...) use underscores that kebab-casing would mangle.
  *
  * Map fields use plain `Map` (zio-blocks derives `Schema[Map]` but not `Schema[ListMap]`); populate them with `ListMap`
  * to keep insertion order deterministic — the derived codec preserves it.
  */
