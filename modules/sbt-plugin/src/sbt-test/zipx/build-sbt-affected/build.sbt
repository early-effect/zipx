...
def shared: Seq[Setting[?]] = Seq(publish / skip := true)

lazy val core = project.settings(shared)
lazy val app  = project.dependsOn(core).settings(name := "app")
