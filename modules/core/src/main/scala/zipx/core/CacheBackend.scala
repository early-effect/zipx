...
enum CacheBackend:
  /** Persist `~/.sbt`, `~/.cache/sbt`, `~/.cache/coursier`, and the build `target/` (compiled classes +
    ...
    * `-SNAPSHOT`) the prior release epoch so the first post-tag PR can warm from the tag build, then any older OS+JDK
    * sbt cache. Disables setup-sbt `disk-cache` and setup-java `cache: sbt` so caching is not also pinned to
    * `hashFiles`.
    */
  case LocalDir
