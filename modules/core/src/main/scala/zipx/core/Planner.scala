...
/** Maps a [[ModuleGraph]] + capabilities + [[PlanConfig]] to a GitHub Actions [[Workflow]].
  ...
  * module), wires `needs` from the graph, expands a per-module Scala matrix, and attaches caching. The result is
  * rendered to YAML by [[zipx.workflow.Render]].
  *
  * M1 implements the [[Capability.test]] path (parallel-with-upstream, local-dir cache). Publish ordering, affected-only
  * gating, and remote caches build on the same structure in later milestones.
  */
object Planner:

  /** The job id for a capability's job on a given module, e.g. `test-schema`. Deterministic and collision-free
    * because sbt project ids are unique.
    */
  ...
  def plan(graph: ModuleGraph, capabilities: List[Capability], config: PlanConfig): Workflow =
    ...
    val capabilityJobs =
      orderedCaps.flatMap {
        case c if c.scope == CapabilityScope.Once => List(onceJob(c, config))
        case c =>
          for
            moduleId <- topoOrder
  ...
  /** The leading setup job: computes the affected module set as a JSON array output consumed by Verify jobs' `if`
    ...
    * (default) or, when `affectedOnPush`, diffs against the push `before` sha (with a guard for force-push / first-push).
  ...
  private def affectedSetupJob(config: PlanConfig): Job =
    Job(
      ...
      steps = List(
        Step(uses = Some("actions/checkout@v4"), `with` = ListMap("fetch-depth" -> "0")),
        Step(uses = Some("sbt/setup-sbt@v1")),
        Step(
          id = Some("compute"),
  ...
  /** A single build-wide job for a `Once` capability (a lint/format gate). No module fan-out, no affected/upstream
    * `needs`; a release gate applies if the capability requests it. Caching and `extraSteps` are honored like any job.
    */
  private def onceJob(capability: Capability, config: PlanConfig): (String, Job) =
  ...
  private def jobsFor(
    ...
    def baseJob(
      ...
      id -> Job(
        ...
        strategy = matrix,
        services = cache.services,
        // Target env is merged after cache env so a target may override cache-provided keys if it must.
        env = cache.env ++ extraEnv,
  ...
  private def jobCondition(
    ...
    val upstreamGuards =
      if gatedOnAffected && upstreamNeeds.nonEmpty then
        upstreamNeeds.sorted.map(u => s"needs.$u.result != 'failure'")
      else Nil
  ...
  /** For dependency-ordered capabilities, contract away non-participating intermediates: walk up the dependency graph
    ...
    */
  private def nearestParticipatingAncestors(node: ModuleNode, graph: ModuleGraph, capability: Capability): List[String] =
  ...
  private def stepsFor(
    ...
    List(
      ...
      Step(
        ...
        uses = Some("actions/setup-java@v4"),
      ...
      case CacheBackend.ManagedRemote(uri, headerSecret) =>
        ...
        CacheContribution(
          env = ListMap(
            ...
            "ZIPX_REMOTE_CACHE_HEADER" -> s"$${{ secrets.$headerSecret }}",
          ),
        )
