...
5. No stringly-typed construction in the public API: every string that reaches YAML comes from a validated Scala value or a linted, warned raw escape hatch. Its teeth: no public API takes a `String` in a position a validated newtype exists for, and a failure is removed at the strongest tier available (unrepresentable > compile-time literal check > `Either` > throw at the sbt boundary) rather than the most convenient one. `grep -rn "throw \|makeOrThrow\|orThrow" modules/*/src/main` is the check, and the only line it may return is `ModuleGraph.apply`, the fixture constructor. A new escape hatch must be typed (so it cannot emit unparseable YAML), report itself (`rawFragments`), and be warned at generate time naming its step or bundle.
