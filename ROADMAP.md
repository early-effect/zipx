...
`zipxActionUpdate yes` / `zipxPinUpdate yes`, `zipxCatalogGenerate` (not workflow YAML; `GITHUB_TOKEN` cannot push `.github/workflows`), `gh pr create` on `zipx/version-updates`. Local `zipxDepUpdate` / `zipxPinUpdate` /
